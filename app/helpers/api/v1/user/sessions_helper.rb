	...
	def find_user_session_by_token(token)
		if token
			UserSession.find_each do |user_session|
				if ActiveSupport::SecurityUtils.secure_compare(user_session.token, token)
					return user_session
				end
			end
		end
	...
	def current_user_session(create_if_nil: false)
		if cookies[:session_token].present?
			if @user_session.present?
				...
				UserSession.find_each do |user_session|
					if ActiveSupport::SecurityUtils.secure_compare(user_session.token, cookies[:session_token])
